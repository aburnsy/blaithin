...
while pageSize <= 1000:
    ...
    driver.get(
        f"https://www.rhs.org.uk/plants/search-results?pageSize={pageSize}&startFrom={startFrom}"
    )
    # print(driver.page_source)
    div1 = driver.find_elements(
        By.XPATH,
        "//*[@id='content']/app-root/app-plant-search-result-page/app-plant-listing/div/div/div/div[2]/app-plant-search-list",
    )
    if len(div1) != 0:
    ...
    for li in lis[0]:
        ...
        if len(outer_path) > 0:
            ...
            title_summ_div = soup.find_all(
                "div", {"class": "l-module__content u-p-l-1-lg"}
            )[0]

            try:
                scientifc_name = (
                    title_summ_div.find_all("h1")[0]
                    .find_all("span")[0]
                    .text.split("\n")[0]
                )
            ...
            try:
                normal_name = title_summ_div.find_all(
            ...
            try:
                description = title_summ_div.find_all(
            ...
            try:
                plant_type = soup.find_all("span", {"class": "label ng-star-inserted"})[
                    0
                ].text.split("\n")[0]
            except:
                plant_type = np.NAN
            ...
            many_to_many_divs = soup.find_all(
                "div", {"class": "plant-attributes__panel"}
            )[1]

            soil_div = many_to_many_divs.find_all(
                "div",
                {
                    "class": "l-row l-row--space l-row--compact l-row--auto-clear ng-star-inserted"
                },
            )
            ...
            try:
                for soil_name in soils:
                    soil_id = soil[
                        soil["soil_type"] == soil_name.text.upper()
                    ].index.values
                    print(soil_id)
                    print(len(plant_table) + 1)
                    soil_temp_df = {
                        "soil_type_id": [int(soil_id[0])],
                        "plant_id": [int(len(plant_table) + 1)],
                    }
                    soil_temp_df = pd.DataFrame(soil_temp_df)
                    # soil_plant = soil_plant.concat(soil_temp_df)
                    soil_plant = pd.concat([soil_plant, soil_temp_df])
            ...
            moist_ph_div = many_to_many_divs.find_all(
                "div", {"class": "l-row l-row--space l-row--auto-clear"}
            )
            ...
            moist = (
                ...
                .find_all(
                    "div",
                    {
                        "class": "l-col-xs-6 l-col-sm-6 l-col-md-6 l-col-lg-6 ng-star-inserted"
                    },
                )[0]
                .find_all("span")
            )
            try:
                for moist_name in moist:
                    ...
                    moist_id = moist_level[
                        moist_level["moist_level"] == moist_name
                    ].index.values
                    print(moist_id)
                    moist_temp_df = {
                        "moist_level_id": [int(moist_id[0])],
                        "plant_id": [int(len(plant_table) + 1)],
                    }
                    moist_temp_df = pd.DataFrame(moist_temp_df)
                    # moist_plant = moist_plant.concat(moist_temp_df)
                    moist_plant = pd.concat([moist_plant, moist_temp_df])
            except:
                moist_id = np.NAN
                moist_temp_df = {
                    "moist_level_id": [moist_id],
                    "plant_id": [int(len(plant_table) + 1)],
                }
                moist_temp_df = pd.DataFrame(moist_temp_df)
                # moist_plant = moist_plant.concat(moist_temp_df)
            ...
            try:
                for ph_name in ph:
                    ...
                    ph_temp_df = {
                        "ph_level_id": [int(ph_id[0])],
                        "plant_id": [int(len(plant_table) + 1)],
                    }
                    ph_temp_df = pd.DataFrame(ph_temp_df)
                    # ph_plant = ph_plant.concat(ph_temp_df)
                    ph_plant = pd.concat([ph_plant, ph_temp_df])
            except:
                ph_id = np.NAN
                ph_temp_df = {
                    "ph_level_id": [ph_id],
            ...
            plant_size_max_height = soup.find_all("div", {"class": "flag__body"})[
                0
            ].text
            ...
            try:
                sun_exposer_list = soup.find_all(
                    "ul", {"class": "list-inline ng-star-inserted"}
                )[0].find_all("li")
                sun_exposer = str()
            ...
            try:
                garden_types = (
                    cultivate.find_all("span", {"class": "ng-star-inserted"})[2]
                    .find_all("ul")[0]
                    .find_all("li")
                )
                plant_how_to_garden = []
            ...
            temp_df = {
                "plant_scientific_name": [scientifc_name],
                "plant_title": [normal_name],
                "plant_description": [description],
                "plant_type": [plant_type],
                "plant_size_max_height_lowest": [plant_size_max_height_lowest],
                "plant_size_max_height_highest": [plant_size_max_height_highest],
                "plant_max_time_lowest": [plant_max_time_lowest],
                "plant_max_time_highest": [plant_max_time_highest],
                "plant_sun_exposer": [sun_exposer],
                "plant_how_to_culitvate": [plant_how_to_culitvate],
                "plant_how_to_propagate": [plant_how_to_propagate],
                "plant_how_to_garden_type": [plant_how_to_garden],
                "plant_how_to_pruning": [plant_how_to_pruning],
                "plant_how_to_pests": [plant_how_to_pests],
            }

            # plant_table = plant_table.concat(pd.DataFrame(temp_df))
            plant_table = pd.concat([plant_table, pd.DataFrame(temp_df)])
