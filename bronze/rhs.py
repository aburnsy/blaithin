...
def extract_detailed_plant_data(plant: dict, session: HTMLSession) -> dict:
    ...
    for k, v in bottom_panel_dict.items():
        print(f"{k:<25}: {v}")
    print(f"Finished processing {plant_url}")
...
def fetch_sample_plants() -> list[dict]:
    ...
    return pl.read_parquet("test\\rhs_urls.parquet").sample(1000).to_dicts()


def get_plants_detail(plants: list[dict] = fetch_sample_plants()) -> list[dict] | None:
    detailed_plants = []
    session = HTMLSession()
    for plant in plants:
        if (
            len(extract := extract_detailed_plant_data(plant=plant, session=session))
            != 0
        ):
